...
import { getChildProcesses, forceKillProcess, waitForProcessesExit, removePidFile } from "./ProcessManager.js";
...
export async function performGracefulShutdown(config: GracefulShutdownConfig): Promise<void> {
  ...
  if (config.server) {
    await withTimeout(closeHttpServer(config.server), SHUTDOWN_STEP_TIMEOUT_MS, "Close HTTP server");
    logger.info("SYSTEM", "HTTP server closed");
  }

  await withTimeout(config.sessionManager.shutdownAll(), SHUTDOWN_STEP_TIMEOUT_MS, "Shutdown sessions");

  if (config.mcpClient) {
    await withTimeout(config.mcpClient.close(), SHUTDOWN_STEP_TIMEOUT_MS, "Close MCP client");
    logger.info("SYSTEM", "MCP client closed");
  }

  if (config.dbManager) {
    await withTimeout(config.dbManager.close(), SHUTDOWN_STEP_TIMEOUT_MS, "Close database");
  }
