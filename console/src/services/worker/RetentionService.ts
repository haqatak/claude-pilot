...
export class RetentionService {
  ...
  async preview(policy?: RetentionPolicy): Promise<RetentionPreview> {
    ...
    const excludeCondition =
      p.excludeTypes.length > 0 ? `AND type NOT IN (${p.excludeTypes.map(() => "?").join(", ")})` : "";
    ...
    if (p.maxCount > 0) {
      const projectsQuery = `
        ...
        HAVING count > ?
      `;
      const projects = db.prepare(projectsQuery).all(...p.excludeTypes, p.maxCount) as Array<{
        project: string;
        count: number;
      }>;
    ...
    return {
      ...
      toDelete: {
        byAge,
        byCount,
        total: Math.min(byAge + byCount, totalObservations - excludedResult.count),
      },
  ...
  async run(policy?: RetentionPolicy, dryRun: boolean = false): Promise<RetentionResult> {
    ...
    try {
      if (p.maxAgeDays > 0) {
        ...
        logger.info(
          "RETENTION",
          `Age-based cleanup: ${p.softDelete ? archived : deleted} observations (cutoff: ${p.maxAgeDays} days)`,
        );
      }
      ...
      if (p.maxCount > 0) {
        ...
        logger.info(
          "RETENTION",
          `Count-based cleanup: ${p.softDelete ? archived : deleted} observations (max: ${p.maxCount} per project)`,
        );
      }
    } catch (error) {
  ...
  async restore(ids?: number[]): Promise<{ restored: number; errors: string[] }> {
    ...
    try {
      const whereClause = ids && ids.length > 0 ? `WHERE id IN (${ids.map(() => "?").join(", ")})` : "";
      ...
      const result = ids && ids.length > 0 ? db.prepare(restoreQuery).run(...ids) : db.prepare(restoreQuery).run();
