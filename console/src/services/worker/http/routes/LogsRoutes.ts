 ...
 * Logs Routes
 *
 * Handles fetching and clearing log files from ~/.pilot/memory/logs/
 */
...
export class LogsRoutes extends BaseRouteHandler {
  private getLogFilePath(): string {
    const dataDir = SettingsDefaultsManager.get("CLAUDE_PILOT_DATA_DIR");
    const logsDir = join(dataDir, "logs");
    const date = new Date().toISOString().split("T")[0];
    return join(logsDir, `pilot-memory-${date}.log`);
  }
