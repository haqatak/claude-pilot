/**
 * SearchManager - Core search orchestration for pilot-memory
 *
 ...
 * - VectorSearchStrategy: Vector-based semantic search (supports Chroma)
...
import { SearchOrchestrator, TimelineBuilder, SEARCH_CONSTANTS } from "./search/index.js";
...
export class SearchManager {
  ...
  constructor(
    ...
    this.orchestrator = new SearchOrchestrator(sessionSearch, sessionStore, vectorSync);
    this.timelineBuilder = new TimelineBuilder();
  }

  /**
   * Query vector database via VectorSync interface
  ...
  private async queryVector(
    ...
    return await this.vectorSync.query(query, limit, whereFilter);
  }
  ...
  private normalizeParams(args: any): any {
    ...
    if (normalized.obs_type && typeof normalized.obs_type === "string") {
      normalized.obs_type = normalized.obs_type
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
    }
  ...
  async search(args: any): Promise<any> {
    ...
    if (!query || query === "*") {
      logger.debug("SEARCH", "Filter-only query (no query text), using direct SQLite filtering", {
        enablesDateFilters: true,
      });
      ...
      const chromaResults = await this.queryVector(query, 100, whereFilter);
      ...
      if (chromaResults.ids.length > 0) {
        ...
        logger.debug("SEARCH", "Hydrated results from SQLite", {
      ...
      logger.debug("SEARCH", "Install UVX/Python to enable vector search", {
    ...
    if (totalResults === 0) {
      ...
      const queryText = query && query !== "*" ? ` matching "${query}"` : "";
      return {
        content: [
          {
            type: "text" as const,
            text: `No results found${queryText}`,
          },
        ],
      };
    }
    ...
    const lines: string[] = [];
    const queryLabel = query && query !== "*" ? ` matching "${query}"` : "";
    lines.push(
      `Found ${totalResults} result(s)${queryLabel} (${observations.length} obs, ${sessions.length} sessions, ${prompts.length} prompts)`,
    );
    lines.push("");
      ...
      for (const [file, fileResults] of resultsByFile) {
        ...
        for (const result of fileResults) {
          if (result.type === "observation") {
            ...
            const formatted = this.formatter.formatSessionSearchRow(
  ...
  async timeline(args: any): Promise<any> {
    ...
    if (query) {
      ...
      if (this.vectorSync) {
        try {
          ...
          logger.debug("SEARCH", "Chroma returned semantic matches for timeline", {
            matchCount: chromaResults?.ids?.length ?? 0,
          });
          ...
          logger.error(
            "SEARCH",
            "Chroma search failed for timeline, continuing without semantic results",
            {},
            chromaError as Error,
          );
        }
      }
      ...
      timelineData = this.sessionStore.getTimelineAroundObservation(
        topResult.id,
    ...
    const items: TimelineItem[] = [
      ...(timelineData.observations || []).map((obs: any) => ({
        type: "observation" as const,
        data: obs,
        epoch: obs.created_at_epoch,
      })),
      ...(timelineData.sessions || []).map((sess: any) => ({
        type: "session" as const,
        data: sess,
        epoch: sess.created_at_epoch,
      })),
      ...(timelineData.prompts || []).map((prompt: any) => ({
        type: "prompt" as const,
        data: prompt,
        epoch: prompt.created_at_epoch,
      })),
    ...
    if (query) {
      ...
      const anchorTitle =
        anchorObs && anchorObs.type === "observation"
    ...
    lines.push(
      `**Window:** ${depth_before} records before -> ${depth_after} records after | **Items:** ${filteredItems?.length ?? 0}`,
    );
    lines.push("");
    ...
    if (this.vectorSync) {
      try {
        if (query) {
          ...
          const chromaResults = await this.queryVector(query, Math.min((filters.limit || 20) * 2, 100), {
            type: "decision",
          });
          ...
          if (metadataResults.length > 0) {
            ...
            const chromaResults = await this.queryVector("decision", Math.min(ids.length, 100));
        ...
        logger.error(
          "SEARCH",
          "Chroma search failed for decisions, falling back to metadata search",
          {},
          chromaError as Error,
        );
      }
    ...
    if (this.vectorSync) {
      try {
        ...
        if (allIds.size > 0) {
          ...
          const chromaResults = await this.queryVector("what changed", Math.min(idsArray.length, 100));
        ...
        logger.error(
          "SEARCH",
          "Chroma search failed for changes, falling back to metadata search",
          {},
          chromaError as Error,
        );
      }
  ...
  async howItWorks(args: any): Promise<any> {
    ...
    if (this.vectorSync) {
      ...
      if (metadataResults.length > 0) {
        const ids = metadataResults.map((obs) => obs.id);
        const chromaResults = await this.queryVector("how it works architecture", Math.min(ids.length, 100));
  ...
  async findByConcept(args: any): Promise<any> {
    ...
    if (this.vectorSync) {
      ...
      if (metadataResults.length > 0) {
        const ids = metadataResults.map((obs) => obs.id);
        const chromaResults = await this.queryVector(concept, Math.min(ids.length, 100));
  ...
  async findByFile(args: any): Promise<any> {
    ...
    if (this.vectorSync) {
      ...
      logger.debug("SEARCH", "Found results for file", {
        file: filePath,
      ...
      if (metadataResults.observations.length > 0) {
        ...
        const chromaResults = await this.queryVector(filePath, Math.min(ids.length, 100));
  ...
  async findByType(args: any): Promise<any> {
    ...
    if (this.vectorSync) {
      ...
      if (metadataResults.length > 0) {
        const ids = metadataResults.map((obs) => obs.id);
        const chromaResults = await this.queryVector(typeStr, Math.min(ids.length, 100));
  ...
  async getTimelineByQuery(args: any): Promise<any> {
    ...
    if (this.vectorSync) {
      ...
      if (chromaResults.ids.length > 0) {
        ...
        if (recentIds.length > 0) {
          results = this.sessionStore.getObservationsByIds(recentIds, {
            orderBy: "date_desc",
    ...
    if (mode === "interactive") {
      ...
      lines.push("");
      lines.push(
        `To get timeline context around any of these observations, use the \`get_context_timeline\` tool with the observation ID as the anchor.`,
      );
      lines.push("");
      ...
      const items: TimelineItem[] = [
        ...
        ...(timelineData.sessions || []).map((sess) => ({
          type: "session" as const,
          data: sess,
          epoch: sess.created_at_epoch,
        })),
