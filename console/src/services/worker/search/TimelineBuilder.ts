...
export class TimelineBuilder {
   ...
   */
  private findAnchorIndex(items: TimelineItem[], anchorId: number | string, anchorEpoch: number): number {
  ...
  formatTimeline(
    ...
    if (depthBefore !== undefined && depthAfter !== undefined) {
      lines.push(
        `**Window:** ${depthBefore} records before -> ${depthAfter} records after | **Items:** ${items.length}`,
      );
    } else {
   ...
   */
  private sortDaysChronologically(dayMap: Map<string, TimelineItem[]>): Array<[string, TimelineItem[]]> {
