 ...
 */
export function renderMarkdownContextEconomics(economics: TokenEconomics, config: ContextConfig): string[] {
  const output: string[] = [];

  output.push(`**Context Economics**:`);
  output.push(
    `- Loading: ${economics.totalObservations} observations (${economics.totalReadTokens.toLocaleString()} tokens to read)`,
  );
  output.push(
    `- Work investment: ${economics.totalDiscoveryTokens.toLocaleString()} tokens spent on research, building, and decisions`,
  );
...
export function renderMarkdownFileHeader(file: string): string[] {
  return [`**${file}**`, `| ID | Time | T | Title | Read | Work |`, `|----|------|---|-------|------|------|`];
}

/**
 * Render markdown table row for observation
 */
export function renderMarkdownTableRow(obs: Observation, timeDisplay: string, config: ContextConfig): string {
...
export function renderMarkdownSummaryItem(
  ...
  return [`**#S${summary.id}** ${summaryTitle}`, ""];
}
...
export function renderMarkdownPreviouslySection(priorMessages: PriorMessages): string[] {
  ...
  return ["", "---", "", `**Previously**`, "", `A: ${priorMessages.assistantMessage}`, ""];
}
