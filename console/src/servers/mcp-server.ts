/**
 * Pilot-Memory MCP Search Server - Thin HTTP Wrapper
 *
 * Refactored from 2,718 lines to ~600-800 lines
 * Delegates all business logic to Worker HTTP API at localhost:41777
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        project: {
          type: "string",
          description: 'Project name (uses "pilot-memory" if omitted)',
        },
      },
      required: ["text"],
    },
...
async function main() {
  ...
  logger.info("SYSTEM", "MCP Search Server started");
