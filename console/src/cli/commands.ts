 ...
 */
async function apiRequest<T>(endpoint: string, options: { method?: string; body?: unknown } = {}): Promise<T> {
...
export async function statusCommand(options: CLIOptions): Promise<void> {
  ...
  const [health, stats, processing] = await Promise.all([
    apiRequest<{ status: string; pid: number; initialized: boolean; coreReady: boolean; mcpReady: boolean }>(
      "/api/health",
    ),
...
export async function searchCommand(query: string, options: CLIOptions): Promise<void> {
  ...
  const results = await apiRequest<{ content: Array<{ type: string; text: string }> }>(`${endpoint}?${params}`);
...
export async function backupCommand(options: CLIOptions): Promise<void> {
  ...
  const result = await apiRequest<{ filename: string; sizeBytes: number; path: string }>("/api/backups/create", {
    method: "POST",
  });
    ...
    case "preview": {
      ...
      if (options.json) {
        ...
        if (preview.affectedProjects.length > 0) {
          console.log("");
          console.log(
            `Affected projects: ${preview.affectedProjects.slice(0, 5).join(", ")}${preview.affectedProjects.length > 5 ? "..." : ""}`,
          );
        }
      }
      break;
