...
export function parseObservations(text: string, correlationId?: string): ParsedObservation[] {
  ...
  while ((match = observationRegex.exec(text)) !== null) {
    ...
    if (cleanedConcepts.length !== concepts.length) {
      logger.debug("PARSER", "Cleaned observation type from concepts", {
        correlationId,
        type: finalType,
        removed: concepts.filter((c) => c === finalType),
      });
    }
  ...
  const deduplicated = observations.filter((obs) => {
    const key = `${obs.type}|${obs.title || ""}`;
    if (seen.has(key)) {
      return false;
    }
    seen.add(key);
  ...
  if (deduplicated.length < observations.length) {
    logger.warn("PARSER", `Deduplicated observations`, {
      correlationId,
      original: observations.length,
      deduplicated: deduplicated.length,
      removed: observations.length - deduplicated.length,
    });
  }
